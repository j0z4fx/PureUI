...
local j=Instance.new"Frame"
j.Name=i.Name or"Dropdown"
j.Size=UDim2.new(1,0,0,38)
...
k.Size=UDim2.new(0.4,-8,0,38)
k.BackgroundTransparency=1
k.Font=Enum.Font.Gotham
k.Text=i.Name or"Dropdown"
...
l.Position=UDim2.new(1,-8,0,7)
l.Size=UDim2.new(0.45,-8,0,24)
l.BackgroundColor3=Color3.fromRGB(45,48,57)
l.BorderSizePixel=0
...
m.PaddingLeft=UDim.new(0,7)
m.PaddingRight=UDim.new(0,22)
m.Parent=l
...
p.Size=UDim2.new(1,-16,0,0)
p.BackgroundColor3=Color3.fromRGB(35,38,46)
p.BorderSizePixel=0
...
table.insert(r.Connections,l.MouseButton1Click:Connect(function()
r:SetOpen(not r.Open)
end))
...
l.Text=i.Placeholder or"Select"
end

return r
end

function d.SetOpen(h,i)
h.Open=i
local j=if i then math.min(#h.Options,g)*f else 0
b:Create(h.Root,e,{Size=UDim2.new(1,0,0,38+j)}):Play()
b:Create(h.List,e,{Size=UDim2.new(1,-16,0,j)}):Play()
b:Create(h.Arrow,e,{Rotation=if i then 180 else 0}):Play()
b:Create(h.Button,e,{
...
assert(k,"PureUI dropdown value is not in Options")

h.Value=i
h.Button.Text=tostring(i)
h:SetOpen(false)

for l,m in pairs(h.OptionButtons)do
m.BackgroundColor3=if l==i then Color3.fromRGB(58,62,73)else Color3.fromRGB(35,38,46)
end

if not j and type(h.Callback)=="function"then
task.spawn(h.Callback,i)
end
return h
end
...
assert(type(i)=="table","PureUI dropdown Options must be a table")
for j,k in ipairs(h.OptionConnections or{})do k:Disconnect()end
h.OptionConnections={}
for j,k in pairs(h.OptionButtons)do k:Destroy()end
h.OptionButtons={}
h.Options=i
...
l.Name=tostring(k)
l.LayoutOrder=j
l.Size=UDim2.new(1,0,0,f)
l.BackgroundColor3=Color3.fromRGB(35,38,46)
l.BorderSizePixel=0
l.AutoButtonColor=false
l.Font=Enum.Font.Gotham
l.Text=tostring(k)
...
table.insert(h.OptionConnections,l.MouseEnter:Connect(function()
...
for i,j in ipairs(h.Connections)do j:Disconnect()end
for i,j in ipairs(h.OptionConnections)do j:Disconnect()end
h.Root:Destroy()
end
...
return i end function a.k():typeof(__modImpl())local b=a.cache.k if not b then b={c=__modImpl()}a.cache.k=b end return b.c end end do local function __modImpl()
